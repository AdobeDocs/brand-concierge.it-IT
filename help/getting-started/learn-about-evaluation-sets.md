---
title: Informazioni sui set di valutazione
description: Scopri come i set di valutazione consentono di verificare se il Brand Concierge risponde correttamente, resta nell’ambito e gestisce le domande complesse in modo sicuro prima del lancio.
topic: Operational Readiness
role: User
level: Beginner
doc-type: Technical Video
duration: 265
last-substantial-update: 2026-09-01T00:00:00Z
jira: KT-22481
source-git-commit: 2255c486351718718d7f729ae7f870446c955793
workflow-type: tm+mt
source-wordcount: '169'
ht-degree: 0%

---

# Informazioni sui set di valutazione

Le valutazioni indicano se il Brand Concierge di Adobe sta rispondendo correttamente, rimanendo nell’ambito e gestendo in modo sicuro domande complesse. Questo video illustra i tre tipi di valutazione (Funzionale, Fuori ambito e Salvaguardia) e mostra come creare un set di valutazione, eseguirlo e rivedere i risultati contrassegnati prima della pubblicazione.

## A chi serve questo video?

* Addetti al marketing e proprietari di contenuti che si preparano al lancio
* Chiunque convalidi le risposte del concierge prima del go-live
* Squadre responsabili della qualità e della sicurezza del portinaio

## Contenuto video

* I tre tipi di valutazione: funzionale, fuori ambito e salvaguardia
* Come creare un set di valutazione: domande generate dall’intelligenza artificiale o caricamento di file CSV
* Come eseguire una valutazione e rivedere il punteggio complessivo e le domande segnalate
* Come confrontare la risposta effettiva del portinaio alla risposta prevista per ogni domanda segnalata

>[!VIDEO](https://video.tv.adobe.com/v/3502668?captions=ita)

Per la documentazione, consulta [Guida di Brand Concierge](../documentation/overview.md).
